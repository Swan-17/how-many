...
(function () {
  ...
  function beerEquivalent(type, delta) {
    const unitsPerDrink = { pints:2.5, bottles:1.5, wines:1.75, cocktails:2, shots:1 };
    return (Number(delta) || 0) * (unitsPerDrink[type] || 0) / 2.5;
  }

  function ensureCard() {
    const page = $('page-stats'), chart = $('weeklyChart');
    ...
    card.id = 'top-venues-card'; card.className = 'card';
    card.innerHTML = '<button type="button" id="top-venues-button" class="btn-secondary" style="width:100%;padding:12px;text-align:left;display:flex;justify-content:space-between;align-items:center;"><strong>TOP VENUES</strong><span id="top-venues-icon">▼</span></button><div id="top-venues-body" class="hidden" style="margin-top:12px;"><div id="top-venues-list"></div><button type="button" id="show-venues-map-btn" class="btn-submit hidden" style="margin-top:12px;">📍 Show on Map</button></div>';
  ...
  async function emailsForScope(scope) {
    ...
    } else if (scope) groupCodes = [scope];
  ...
  async function loadTopVenues() {
    const card = ensureCard(), select = $('analytics-group-select'), body = $('top-venues-body'), list = $('top-venues-list');
    ...
    try {
      ...
      const { data: sessions, error: se } = await statsSb.from('drinking_sessions').select('id,venue_name,venue_address,latitude,longitude,started_at').in('user_email', emails).order('started_at', { ascending:false });
      if (se) throw se;
      if (!sessions?.length) { list.innerHTML = '<div style="color:var(--text-muted);font-size:12px">No venues visited yet.</div>'; return; }
      const { data: events, error: ee } = await statsSb.from('drink_location_events').select('session_id,drink_type,delta').in('session_id', sessions.map(x => x.id));
      if (ee) throw ee;
      const bySession = {};
      (events || []).forEach(x => { bySession[x.session_id] = (bySession[x.session_id] || 0) + beerEquivalent(x.drink_type, x.delta); });
      const venues = {};
      sessions.forEach(s => {
        const key = `${s.venue_name || 'Unknown venue'}|${s.venue_address || ''}`;
        if (!venues[key]) venues[key] = { name:s.venue_name || 'Unknown venue', address:s.venue_address || '', drinks:0, latitude:Number(s.latitude), longitude:Number(s.longitude) };
        venues[key].drinks += bySession[s.id] || 0;
      });
      const rows = Object.values(venues).map(v => ({...v, drinks:Math.max(0,v.drinks)})).filter(v => v.drinks > 0).sort((a,b) => b.drinks-a.drinks || a.name.localeCompare(b.name));
      mapData = rows.filter(v => Number.isFinite(v.latitude) && Number.isFinite(v.longitude));
      $('show-venues-map-btn')?.classList.toggle('hidden', !mapData.length);
      if (!rows.length) { list.innerHTML = '<div style="color:var(--text-muted);font-size:12px">No beers have been recorded at a venue yet.</div>'; return; }
      list.innerHTML = rows.map(v => `<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:11px 0;border-bottom:1px solid var(--border-color)"><div style="min-width:0;flex:1"><div style="font-size:13px;font-weight:700">${esc(v.name)}</div>${v.address ? `<div style="margin-top:3px;color:var(--text-muted);font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(v.address)}</div>` : ''}</div><div style="color:var(--primary-color);font-size:13px;font-weight:800;white-space:nowrap">${Number(v.drinks.toFixed(1))} beer${Math.abs(v.drinks - 1) < 0.001 ? '' : 's'}</div></div>`).join('');
    } catch (error) {
      if (request === requestId) { list.innerHTML = `<div style="color:#fca5a5;font-size:12px">${esc(error.message || 'Could not load venues.')}</div>`; $('show-venues-map-btn')?.classList.add('hidden'); }
    }
  }

  function toggleTopVenues() {
    const body = $('top-venues-body'), icon = $('top-venues-icon');
    if (!body) return;
  ...
  function ensureMapPage() {
    let page = $('page-venue-map');
    if (page) return page;
    page = document.createElement('div'); page.id = 'page-venue-map'; page.className = 'hidden';
    page.innerHTML = '<div id="venue-map-canvas"></div><div id="venue-map-controls"><div id="venue-map-title">Top Venues</div><button type="button" id="venue-map-return" class="btn-secondary">← Return to Stats</button></div>';
    $('app-screen')?.appendChild(page);
    const style = document.createElement('style'); style.id = 'top-venues-map-styles'; style.textContent = `
      #page-venue-map{position:fixed;inset:0;z-index:5000;background:var(--bg-main);display:flex;flex-direction:column;padding:0;margin:0;max-width:none}
      #page-venue-map.hidden{display:none!important} #venue-map-canvas{flex:1 1 auto;min-height:0;width:100%}
  ...
  function loadLeaflet() {
    ...
    leafletPromise = new Promise((resolve, reject) => {
      if (!$('top-venues-leaflet-css')) { const css=document.createElement('link'); css.id='top-venues-leaflet-css'; css.rel='stylesheet'; css.href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'; document.head.appendChild(css); }
      const script=document.createElement('script'); script.src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'; script.onload=resolve; script.onerror=()=>reject(new Error('Could not load the map library.')); document.head.appendChild(script);
    });
    return leafletPromise;
  }

  async function showVenueMap() {
    if (!mapData.length) return;
    ensureMapPage();
    ['page-stats','page-account','page-tracker','page-admin'].forEach(id => $(id)?.classList.add('hidden'));
    $('page-venue-map')?.classList.remove('hidden'); $('app-screen')?.querySelector('nav')?.classList.add('hidden');
    try {
  ...
  function returnToStats() {
    if (mapInstance) { mapInstance.remove(); mapInstance=null; }
    $('page-venue-map')?.classList.add('hidden'); $('app-screen')?.querySelector('nav')?.classList.remove('hidden');
    if (typeof window.switchPage==='function') window.switchPage('stats'); else $('page-stats')?.classList.remove('hidden');
  }

  window.toggleTopVenues=toggleTopVenues; window.showVenueMap=showVenueMap; window.returnToVenueStats=returnToStats;

  function install() {
    const card=ensureCard(), select=$('analytics-group-select'); if (!card||!select) return;
    syncVisibility();
    const button=$('top-venues-button'); if(button&&!button.__topVenuesBound){button.addEventListener('click',toggleTopVenues);button.__topVenuesBound=true;}
    const mapButton=$('show-venues-map-btn'); if(mapButton&&!mapButton.__mapBound){mapButton.addEventListener('click',showVenueMap);mapButton.__mapBound=true;}
