const CACHE_NAME = 'beer-tracker-v7';
const urlsToCache = ['./', './index.html', './manifest.json', './group-password-ui.js?v=7'];
...
self.addEventListener('fetch', event => {
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request).then(async response => {
      ...
      const injected = html.replace('</body>', '<script src="./group-password-ui.js?v=7"></script></body>');
  ...
  if (new URL(event.request.url).pathname.endsWith('/group-password-ui.js')) {
    event.respondWith(fetch(event.request, { cache: 'no-store' }).catch(() => caches.match('./group-password-ui.js')));
    return;
  }
